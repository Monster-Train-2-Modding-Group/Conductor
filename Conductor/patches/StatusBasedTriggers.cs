...
namespace Conductor.Patches
{
    ...
    class CharacterState_AddStatusEffect_StatusBasedTriggers
    {
        private static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
        {
            ...
            var additionalStatusBasedTriggersMethod = AccessTools.Method(typeof(CharacterState_AddStatusEffect_StatusBasedTriggers), nameof(CharacterState_AddStatusEffect_StatusBasedTriggers.AdditionalStatusBasedTriggers));
            ...
            List<CodeInstruction> newInstructions = [
                ...
                new CodeInstruction(OpCodes.Call, additionalStatusBasedTriggersMethod),  // AdditionalStatusBasedTriggers(CharacterState, string, int, StatusEffectStack, AllGameManagers)
            ];
    ...
    [HarmonyPatch(typeof(CharacterState), nameof(CharacterState.RemoveStatusEffect), [typeof(string), typeof(int), typeof(CharacterState.RemoveStatusEffectParams), typeof(bool)])]
    class CharacterState_RemoveStatusEffect_StatusBasedTriggers
    {
        private static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
        {
            var codes = new List<CodeInstruction>(instructions);
            ...
            var additionalStatusBasedTriggersMethod = AccessTools.Method(typeof(CharacterState_RemoveStatusEffect_StatusBasedTriggers), nameof(CharacterState_RemoveStatusEffect_StatusBasedTriggers.AdditionalStatusBasedTriggers));
            var allGameManagersField = AccessTools.Field(typeof(CharacterState), "allGameManagers");
            ...
            List<CodeInstruction> newInstructions = [
                ...
                new CodeInstruction(OpCodes.Ldarg_1),                                    // string statusId
                new CodeInstruction(OpCodes.Ldarg_2),                                    // int numStacks
                new CodeInstruction(OpCodes.Ldloc_S, 3),                                 // StatusEffectStack statusEffectStack2
                new CodeInstruction(OpCodes.Ldarg_3),                                    // RemoveStatusEffectParams removeParams
                new CodeInstruction(OpCodes.Ldarg_0),
                new CodeInstruction(OpCodes.Ldfld, allGameManagersField),                // AllGameManagers
        ...
        public static void AdditionalStatusBasedTriggers(CharacterState character, string statusId, int numStacks, StatusEffectStack statusEffect, CharacterState.RemoveStatusEffectParams removeParams, AllGameManagers allGameManagers)
        {
            TriggerOnStatusRemovedParams triggerParams = new()
            {
                StatusId = statusId,
                Character = character,
                NumStacks = numStacks,
            ...
            foreach (var trigger_func in CharacterTriggerExtensions.TriggersOnStatusRemoved)
            {
                if (trigger_func.Value(triggerParams, out var queueTriggerParams))
                {
                    allGameManagers.GetCombatManager()!.QueueCustomTrigger(character, trigger_func.Key, queueTriggerParams);
